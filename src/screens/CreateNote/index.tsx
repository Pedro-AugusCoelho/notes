...
const schema: yup.ObjectSchema<CreateNoteFormData> = yup.object({
...
export default function CreateNote() {
  ...
  async function onSubmit(data: CreateNoteFormData) {
    try {
      ...
      // Se for bloqueada, verificar se há senha definida
      if (isSecret) {
  ...
  async function handlePasswordSubmit() {
    ...
    try {
      ...
      const data = control._formValues as CreateNoteFormData
      setIsSaving(true)
      await createNote({
        title: data.title,
        description: data.description,
